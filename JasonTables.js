...
MDD.apis.JasonTables = function (options, records) {
	...
	function objectTraverser (path, obj) {
		...
		for (var i = 1; i < nestingLength; i++) {
			...
			if (MDD.utils.checkVariableType(value) === 'object') {
...
//visible: (!advancedFilter.isActive() && !advancedFilter.isMaximized()) && recordsToDisplay().length > 0 || filter
		...
		navigation: {
			...
			fire: function () {
				...
				if (resultSetLength !== 0 && resultSetLength <= index) {
					...
					MDD.core.log('log', 'is this a problem?');
	...
	var initialize = function () {
		...
		//	advancedSearchInstance = new MDD.apis.AdvancedFilter({
		...
		if (MDD.utils.checkVariableType(options.advancedFilter) === 'object') {
			// not sure I like this approach or use pub/sub
			advancedSearchInstance = new MDD.apis.AdvancedFilter({
				allRecords: allRecords,
				resultSet: resultSet,
